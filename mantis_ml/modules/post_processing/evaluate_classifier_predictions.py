...
class ClassifierEvaluator:
	def __init__(self, cfg, clf_id):
		...
		try:
			...
			print('Non defined classifier {}: Instantiated only basic ClassifierEvaluator skeleton.'.format(clf_id))
	...
	def plot_feat_imp_distribustion(self):
		...
		_ = ax.set_xticks(list(range(len(pivot_feat_df.columns.values))))
	...
	def gene_ranking_boxplot(self, genelist, list_id):
		...
		xtick_labels = []
		xticks = [] 
		for itick, gene in enumerate(genelist):
	...
	def process_gene_proba_predictions(self, top_hits=50, save_to_file=False, top_hits_to_save=1000,
		...
		proba_df_file = self.cfg.superv_proba_pred / (self.clf_id + '.all_genes.predicted_proba.csv')
		try:
			self.gene_proba_df = pd.read_csv(proba_df_file, index_col = 0)
			self.gene_proba_df = self.gene_proba_df.transpose()
			# gene_mean_proba_series = gene_proba_df.predicted_proba
			print('Completed reading of ' + self.clf_id + '.all_genes.predicted_proba.csv')
		...
		self.known_gene_proba_df = self.gene_proba_df[list(set(list(self.known_genes)).intersection(set(list(self.gene_proba_df.columns.values))))]
		...
		if save_to_file:
			self.known_gene_proba_df.to_csv(
				self.cfg.superv_proba_pred / (self.clf_id + '.known_genes.predicted_proba.csv'))
			print('Completed writing of file', self.clf_id + '.known_genes.predicted_proba.csv')
		if make_plots:
		...
		unlabelled_genes = np.array(list(set(list(unlabelled_genes)).intersection(set(list(self.gene_proba_df.columns.values)))))
		...
		if save_to_file:
			self.unlabelled_gene_proba_df.to_csv(
				self.cfg.superv_proba_pred / (self.clf_id + '.unlabelled_genes.predicted_proba.csv'))
			print('Completed writing of file', self.clf_id + '.unlabelled_genes.predicted_proba.csv')
		if make_plots:
