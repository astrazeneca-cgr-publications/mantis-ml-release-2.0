...
class MergePredictionsFromClassifiers:
	...
	def get_definitive_gene_predictions_overlap(self, all_clf, genes_attr='predicted_known_genes', gene_class='Known'):
		...
		top_classifiers = ['ExtraTreesClassifier', 'RandomForestClassifier', 'XGBoost', 'SVC', 'DNN', 'GradientBoostingClassifier', 'Stacking', 'GCN']
	...
	def beta_get_gene_predictions_based_on_thres(self, all_clf, thres=0.5, genes_attr='known_gene_proba_means', gene_class='Known'):
		...
		top_classifiers = ['ExtraTreesClassifier', 'RandomForestClassifier', 'XGBoost', 'SVC', 'DNN', 'GradientBoostingClassifier', 'GCN']
	...
	def merge_proba_from_all_clf(self):
		...
		genes_ranked = self.mean_gene_proba_df_per_clf.mean(axis=1).sort_values(ascending=False).index.values
		merged_clf_proba_df = self.mean_gene_proba_df_per_clf.reindex(genes_ranked)
		merged_clf_proba_df.to_csv(self.cfg.superv_proba_pred / 'AllClassifiers.Merged.all_genes.predicted_proba.csv')
		

# 		merged_clf_proba_df = pd.DataFrame()
# 		for clf_id in self.cfg.classifiers:

# 			proba_df_file = self.cfg.superv_proba_pred / (clf_id + '.all_genes.predicted_proba.csv')
# 			gene_proba_df = pd.read_csv(proba_df_file, index_col = 0)
# 			gene_proba_df = gene_proba_df.sort_values(['predicted_proba'], ascending=False)
...
# 			merged_clf_proba_df = merged_clf_proba_df.reindex(merged_clf_proba_df.mean().sort_values(ascending=False).index, axis=1)
	...
	def merge_mean_proba_from_all_clf(self):
		...
		self.mean_gene_proba_df_per_clf = mean_gene_proba_df_per_clf
	...
	def run(self):
		...
		merged_clf_proba_df = merged_clf_proba_df.transpose()
		top_hits = 50
		...
		consensus_known_genes = [x for x in consensus_known_genes if x in merged_clf_proba_df.columns.values]
		consensus_novel_genes = [x for x in consensus_novel_genes if x in merged_clf_proba_df.columns.values]
