...
for clf_id in ${classifiers[@]}; do
	...
	then
		for final_lvl_clf in DNN; do
			...
			sbatch --priority=TOP -J ${pheno}_$job_id -o ${pheno}_${job_id}_out.txt -e ${pheno}_${job_id}_err.txt --time=${time} --mem-per-cpu=${mem} --cpus-per-task=${cores} ./run_pu.sh $clf_id $final_lvl_clf
		done
	else
		echo -e "\n> $clf_id:"
		echo "sbatch -J ${pheno}_${clf_id} -o ${pheno}_${clf_id}_out.txt -e ${pheno}_${clf_id}_err.txt --time=${time} --mem-per-cpu=${mem} --cpus-per-task=${cores} ./run_pu.sh $pheno $clf_id"
		sbatch --priority=TOP -J ${pheno}_${clf_id} -o ${pheno}_${clf_id}_out.txt -e ${pheno}_${clf_id}_err.txt --time=${time} --mem-per-cpu=${mem} --cpus-per-task=${cores} ./run_pu.sh $pheno $clf_id
	fi
done
