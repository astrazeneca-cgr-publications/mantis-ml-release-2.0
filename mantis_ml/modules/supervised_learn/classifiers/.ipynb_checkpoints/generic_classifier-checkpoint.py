...
# 		X_train_full = self.data.drop(['known_gene', 'Gene_Name'], axis=1).values
# 		y_train_full = self.data.known_gene.values
# 		X_test_full = X_train_full
# 		y_test_full = y_train_full
		
# 		sample_weight = self.data.Gene_Name.isin(self.train_gene_names.values)
# 		in_testing = ~sample_weight
		
# 		self.test_gene_names = self.data.loc[in_testing].Gene_Name
		
# 		# self.model.fit(self.X_train, self.y_train)
# 		self.model.fit(X_train_full[sample_weight], y_train_full[sample_weight])

# 		# self.y_prob = self.model.predict_proba(self.X_test)
# 		self.y_prob = self.model.predict_proba(X_test_full[in_testing])
# 		self.y_pred = np.argmax(self.y_prob, axis=1)

# 		# convert target variables to 2D arrays
# 		# self.y_test = np.array(to_categorical(self.y_test, 2))
# 		self.y_test = np.array(to_categorical(y_test_full, 2))[in_testing]
...
# 		TP_df = conc_pred_df.loc[
# 			(conc_pred_df['pred_1'] >= conc_pred_df['pred_0']) & (conc_pred_df['test_1'] == 1.0)]
# 		FP_df = conc_pred_df.loc[
# 			(conc_pred_df['pred_1'] >= conc_pred_df['pred_0']) & (conc_pred_df['test_1'] == 0.0)]

# 		TN_df = conc_pred_df.loc[
# 			(conc_pred_df['pred_0'] > conc_pred_df['pred_1']) & (conc_pred_df['test_0'] == 1.0)]
# 		FN_df = conc_pred_df.loc[
# 			(conc_pred_df['pred_0'] > conc_pred_df['pred_1']) & (conc_pred_df['test_0'] == 0.0)]
